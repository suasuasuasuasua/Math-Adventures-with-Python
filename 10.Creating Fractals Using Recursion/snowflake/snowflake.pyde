...
def draw():
    ...
    size = map(mouseY,0,height,0,400) # mouse-Y determine the size
    
    translate(-size/2+width/2,-size/(2*sqrt(3))+height/2) # centers the triangle perfectly
...
def segment(size, level):
    if level == 0: # default case just draws a line 
        ...
        # segments of the same level are the same size
        segment(size/3.0, level-1)
        # line(0,0,size/3.0,0) # draws a line that is one-third of the size
        # translate(size/3.0,0) # moves to the end of the line
