...
def draw():
  ...
  # defines a time varying rotation matrix
  # this matrix will depend on the position of our mouse cursor
  ang = map(mouseX, 0, width, 0, TWO_PI)
  rot_matrix = [ [cos(ang), -sin(ang)],
                 [sin(ang), cos(ang) ] ]

  time_matrix = [ [cos(t), -sin(t)],
                  [sin(t), cos(t)] ]

  strokeWeight(2) # a slightly thicker line
  stroke(0) # sets line color to black

  rotatedMatrix = transpose(multMatrix( rot_matrix, transpose(fmatrix) ) ) # this one depends on the mouse cursor
  timeMatrix = transpose(multMatrix( time_matrix, transpose(fmatrix) ) ) # this one depends on a time variable
  graphPoints(fmatrix)
